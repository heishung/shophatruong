...
ob_start();
    ...
    ?>
  <a style="font-size:12px;" class="nj-facebook-messenger <?php echo $id ?>_open" href="#"><?php if( $array["button"] == "false"){?><?php echo get_option("facebook_messenger_text_botton") ?> <?php }else{ echo $array["button"];} ?></a>
    <?php }else{
        $id = $array["id"];
    } ?>
  <div id="<?php echo $id ?>" class="facebook_messenger_popup">
        ...
        <div class="send-app">
            ...
            <a href="https://m.me/<?php echo $ms[1] ?>"><?php _e("Send message via your Messenger App","fb_messenger") ?></a>
        </div>
        <?php endif; ?>
  </div>
